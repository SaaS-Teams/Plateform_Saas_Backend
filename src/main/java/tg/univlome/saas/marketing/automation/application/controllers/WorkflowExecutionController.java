...
public class WorkflowExecutionController {
    ...
    @PostMapping
    @Operation(summary = "Démarrer une nouvelle exécution (Test/Manuel)", description = "Permet de forcer l'entrée d'un contact dans un scénario. Normalement, cette action est déclenchée automatiquement par des événements système, mais cet endpoint est utile pour les tests ou les déclenchements manuels.")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Vérifier le statut d'une exécution", description = "Récupère les détails d'un parcours spécifique (où est bloqué le contact, a-t-il reçu l'email, y a-t-il une erreur ?).")
    ...
    @Operation(summary = "Lister les participants d'un scénario", description = "Récupère la liste de tous les contacts qui sont en train de parcourir (ou ont terminé) un scénario spécifique. Utile pour les statistiques de la campagne.")
    ...
    @Operation(summary = "Historique d'automation d'un contact", description = "Récupère tous les scénarios par lesquels un contact spécifique est passé (historique CRM).")
