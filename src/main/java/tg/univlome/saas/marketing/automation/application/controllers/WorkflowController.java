...
public class WorkflowController {
    ...
    @PostMapping
    @Operation(summary = "Créer un nouveau scénario", description = "Enregistre un nouveau parcours d'automatisation. Le champ 'flowData' doit contenir le JSON brut généré par l'éditeur visuel (React Flow par exemple).")
    @ApiResponses(value = {
    ...
    @GetMapping
    @Operation(summary = "Lister tous les scénarios", description = "Récupère la liste de toutes les campagnes d'automatisation (Brouillons, Actives et En Pause) pour l'affichage dans le tableau de bord.")
    public ResponseEntity<List<WorkflowResponse>> getAllWorkflows() {
        return ResponseEntity.ok(workflowService.getAllWorkflows());
    }

    @GetMapping("/{trackingId}")
    @Operation(summary = "Obtenir les détails d'un scénario", description = "Récupère la structure complète d'un parcours (incluant son flowData JSON) via son UUID public. Idéal pour recharger un parcours dans l'éditeur visuel.")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Mettre à jour un scénario", description = "Écrase la définition d'un scénario existant. Utilisé lorsque l'utilisateur clique sur 'Sauvegarder' dans l'éditeur visuel.")
    @ApiResponses(value = {
